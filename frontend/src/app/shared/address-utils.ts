    ...
    if (this.type === 'v1_p2tr') {
      for (let i = 0; i < vin.length; i++) {
        ...
        if (v.inner_witnessscript_asm) {
          ...
          const controlBlock = hasAnnex
            ? v.witness[v.witness.length - 2]
            : v.witness[v.witness.length - 1];
          const scriptHex = hasAnnex
            ? v.witness[v.witness.length - 3]
            : v.witness[v.witness.length - 2];
          this.processScript(
            new ScriptInfo(
              'inner_witnessscript',
...
export function compareAddressInfo(
  ...
  const prefixScore = isBase58
    ? 1
    : ADDRESS_PREFIXES[a.network || 'mainnet'].bech32.length;
