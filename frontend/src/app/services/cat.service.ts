...
import { CAT20, CAT721, ConstantsLib, CatTags } from '@opcat-labs/cat-sdk'
import { Address, Script, hash256, sha256, ContractHeaderSerializer} from '@opcat-labs/scrypt-ts-opcat'
...
export class CatService {
  ...
  detectAOutputIsCat20(vout: Vout): boolean {
    try {
      const tags = this.decodeScriptTags(vout.scriptpubkey);
      if (!tags.includes(CatTags.CAT20_TAG)) {
        return false;
      }
      CAT20.deserializeState(vout.data);
      return true;
    } catch (e) {
      return false;
    }
  ...
  detectAOutputIsCat721(vout: Vout): boolean {
    try {
      const tags = this.decodeScriptTags(vout.scriptpubkey);
      if (!tags.includes(CatTags.CAT721_TAG)) {
        return false;
      }
      CAT721.deserializeState(vout.data);
    } catch (e) {
      return false;
    }
    return false;
